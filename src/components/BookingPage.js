...
const BookingPage = () => {
  ...
  useEffect(() => {
    const fetchPlannerDetails = async () => {
      try {
        const plannerResponse = await axios.get(`http://localhost:5000/planners/${plannerId}`);
        setPlanner(plannerResponse.data);

        const reviewsResponse = await axios.get(`http://localhost:5000/reviews?plannerId=${plannerId}`);
